...
class ConfigManager:
    ...
    def read_config_file(
        path: str, make_IfNotExist: bool = False, strict: bool = True
    ) -> AppConfig:
        """read json file data
    ...
    def read_multiple_config_files(*paths) -> AppConfig:
        ...
        for i, path in enumerate(paths):
            if ".json" in path:
                ...
                config_raw: dict = importlib.import_module(
        ...
        try:
            ...
            logger.error(
                f"exception thrown in config validation, maybe all required fields hasn't passed yet: \n {e}"
            )
            raise e
        return config
    ...
    async def async_read_config_file(
        path: str, make_IfNotExist: bool = False
    ) -> AppConfig:
        import pathlib
    ...
    async def async_overwrite_config_file(new_config: AppConfig | dict):
        ...
        conf_to_save = {
            key: val
            for key, val in new_conf_filtered.items()
            if key in config_pub.keys()
        }
